...
[Data here](data/fundamentals.csv).
...
poll is an LV poll, and don't distinguish between RV/A/V polls.
...
\end{align*}
$$
where $i$ indexes the polls, $f[i]$ is the firm, $t[i]$ is the year, $m[i]$ is the methodology, $v[i]$ is the survey population, $m_{f[i]}$ is the herding variable for each firm, and $X$ is a matrix of poll variance predictors: $\sqrt{N_i}$, $\log(\text{time to elec.})$, and the LV/not indicator.
Further details, including the weakly informative priors on all the parameters, may be found in the [Stan model code](stan/firms.stan).

We can simulate from the model to get *predictive* values of firm bias and variance in hypothetical election-day likely voter polls for a future election.
These predictive values are the best way to evaluate each firm's overall quality.
A firm is better---that is, its polls contain more information about the race---if it has lower variance (std. dev.), a lower herding value, and bias closer to 0 (though this can be adjusted for).
...
d_contour = crossing(bias=with(d_firms, seq(min(bias)*1.1, max(bias)*1.1, 0.002)),
...
d_firms |>
    ...
    arrange(herding) |>
ggplot(aes(bias, stdev, label=lab, color=herding, size=n)) +
