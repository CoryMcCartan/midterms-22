...
let mmv = function(e, d) {
    ...
    let miss;
    if (prob > 0.5) {
        cand = d.match.dem_cand == "NA" ? "Democrats" : d.match.dem_cand;
        miss = d.match.dem_cand == "NA";
    } else {
        cand = d.match.rep_cand == "NA" ? "Republicans" : d.match.rep_cand;
        miss = d.match.rep_cand == "NA";
    }
    let inc = {
...
viewof tab_distr = {
    ...
    let fmt_cands = function(x) {
        ...
        let d_cand = x[0] == "NA" ? na_html : x[0];
        let r_cand = x[1] == "NA" ? na_html : x[1];
        if (SMALL) {
            if (x[2] == "dem") {
                d_cand += "*"
            } else if (x[2] == "rep") {
    ...
    let cols = ["name_short", "cands", "inc_seat", "pr_dem", "interval", "part_base", ];
    let cols_small = ["name_short", "cands", "pr_dem", "interval", "part_base", ];
    let cols_tiny = ["name_short", "cands", "pr_dem", "interval", ];
    ...
    return Inputs.table(search, {
        ...
        header: {
            ...
            inc_seat: "Inc.",
            cands: "Candidates",
            pr_dem: small ? "Prob." : "Win prob.",
            interval: small ? "Est. vote" : "Vote forecast",
            part_base: small ? "2020" : "2020 pres.",
        },
...
The <b>Impact</b> column roughly measures how adding the poll to the model moved
the forecast towards Democrats or Republicans.
